...
/// <summary>
/// Power-series dispersion in the squared index: <c>n(λ)² = a₀ + Σ aᵢ λ^pᵢ</c>, with λ in micrometers and up to
/// <see cref="MaximumTermCount"/> terms.
/// </summary>
/// <remarks>
/// <para>This is the "SCHOTT formula" that older catalogs and many current CDGM, Hikari, HOYA and Sumita entries use, typically
/// with exponents 2, −2, −4, −6, −8. Any finite exponent is accepted; even integer exponents are evaluated without
/// <see cref="Math.Pow"/>.</para>
...
/// </remarks>
public readonly struct Polynomial : IDispersionModel, IEquatable<Polynomial>
{
    ...
    {
        DispersionStatus status = series.Evaluate(wavelengthNanometers, out double squared);
    ...
    /// <inheritdoc />
    public bool Equals(Polynomial other) => series.Equals(other.series);

    /// <inheritdoc />
    public override bool Equals(object? obj) => obj is Polynomial other && Equals(other);
